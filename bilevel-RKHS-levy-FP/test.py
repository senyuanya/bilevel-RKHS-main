...
from generateData.generatePDFdata import generatePDFdata
from generateData.generateData import generateObservations
...
u_Type = 'Laplacepdf'  # types: 'Bspline', 'Fourier','Compoundpdf',Laplacepdf; Fourier works for mfOpt, which requires derivatives of u.
...
kernel_type = 'Laplacejump'  # Gaussian, sinkx, FracLap,Compoundlevy,Laplacejump
r0 = 0.05 #cutoff distance of singular levy kernel
drift_func = lambda x: np.sin(x) #np.sin(x)  #  -0.5*x

obsInfo, kernelInfo, p_val, f_val, scale_val = generatePDFdata(x_range, t_range, dx, dt, r0, kernel_type, example_type, drift_func)

######==================== Generate the PDF Data with Finite Difference Method ======================== #######
factor_t = 1  # e.g., use every 5th time point
factor_x = 2 # e.g., use every 2nd spatial point; factor_x = 2;4;5;8
obsInfo, ux_valtr_coarse, fx_valtr_coarse, ux_valts_coarse, fx_valts_coarse, p_val_coarse, f_val_coarse_new = generateObservations(obsInfo, p_val, N, factor_x, factor_t, scale_val, drift_func)
x_mesh_coarse = obsInfo['x_mesh_coarse'] 

fx_valtr_coarse1 = f_val[obsInfo['train_ind'],:]/scale_val[obsInfo['train_ind']].reshape(-1,1)

fx_valts_coarse1 = f_val[obsInfo['test_ind'],:]/scale_val[obsInfo['test_ind']].reshape(-1,1)

####==================== Check if "x_mesh_coarse" is odd symmetric. If not, please reselect the factor_x ============= #######

def is_origin_symmetric_odd(a, tol=1e-8):
    """
    Check if a 1D array is odd symmetric, i.e., f(-x) = -f(x).
    ...
    Returns:
        True if the array is odd symmetric within the given tolerance; False otherwise.
    """
    n = len(a)
    mid = n // 2  # integer division for index
    
    if n % 2 == 0:
        # For even length arrays, split equally.
        left = a[:mid]
        right = a[mid:][::-1]  # reverse the right half
    else:
        # For odd length arrays, ignore the middle element (assumed to correspond to x=0)
        left = a[:mid]
        right = a[mid+1:][::-1]
    
    # Check if left side equals the negative of the right side
    return np.allclose(left, -right, atol=tol)

print("Odd symmetric:", is_origin_symmetric_odd(x_mesh_coarse))
if not is_origin_symmetric_odd(x_mesh_coarse):
    print("The array is not odd symmetric. Please reselect factor_x")
    sys.exit()  # This will stop further execution of the script.

dx_coarse = round(x_mesh_coarse[1] - x_mesh_coarse[0],3) # space mesh size for sampling data
print("dx_coarse:",dx_coarse)

#============PDF part ====================
plt.figure(figsize=(8, 6),dpi=500)

plt.scatter(obsInfo['x_mesh_data'], p_val[-2,:]/scale_val[-2], label=r'original PDF', color='green')
plt.scatter(x_mesh_coarse, p_val_coarse[-2,:], label=r'Coarse PDF', color='thistle', s=20)
...
plt.scatter(obsInfo['x_mesh_data'], f_val[-1,:]/scale_val[-1], label=r'original fx_val', color='green')

plt.scatter(x_mesh_coarse, f_val_coarse_new[-1,:], label=r'Coarse fx_val One', color='thistle', s=20)

plt.title(r'$\int [p(x+y, t) - p(x,t)] g(|y|) dy$ ')
...
bdry_width = int(np.floor((obsInfo['delta'] + 1e-10) / dx_coarse))  # boundary space for interaction range with kernel
r_seq = dx_coarse * np.arange(1, bdry_width + 1)
Index_xi_inUse = np.arange(bdry_width, len(x_mesh_coarse) - bdry_width) # index that x_i in use for L_phi[u]

#============Nonlocal part ====================
indices = np.where((obsInfo['x_mesh_data'] >= x_mesh_coarse[Index_xi_inUse][0]) & (obsInfo['x_mesh_data'] <= x_mesh_coarse[Index_xi_inUse][-1]))[0]
plt.figure(figsize=(8, 6),dpi=500)
plt.scatter(obsInfo['x_mesh_data'][indices], f_val[obsInfo['train_ind'][-1],:][indices]/scale_val[obsInfo['train_ind'][-1]], label=r'original fx_val', color='green', s=30)
plt.scatter(x_mesh_coarse[Index_xi_inUse], f_val_coarse_new[obsInfo['train_ind'][-1],:][Index_xi_inUse], label=r'Coarse fx_val', color='thistle', s=20)
plt.scatter(x_mesh_coarse[Index_xi_inUse], fx_valtr_coarse[-1,:][Index_xi_inUse], label=r'Used Coarse fx_val ', color='tomato', s=10)
...
data_str = f"{obsInfo['example_type']}{kernelInfo['kernel_type']}{u_Type}{obsInfo['x_mesh_coarse_str']}_dx_coarse{dx_coarse:.4f}_"
regressionData = getData4regression_auto(ux_valtr_coarse, fx_valtr_coarse, ux_valts_coarse, fx_valts_coarse, dx_coarse, obsInfo, bdry_width, Index_xi_inUse, r_seq, data_str, normalizeOn)
...
c_reg2, res2, eta2, reg_corner = Tikh_Lcurve(A, f, G_D1, method)
x_reg2 = G_D1 @ c_reg2
...
# Gbar-RKHS
method = 'Tikh'  # method can be 'tsvd', 'Tikh', 'dsvd'
c_reg6, res6, eta6, reg6_corner = Tikh_Lcurve(A, f, G_D1, method)
x_reg6 = G_D1 @ c_reg6

c_reg8, G8, reg8_corner = Tikh_GCV(A, f, G_D1, method)
x_reg8 = G_D1 @ c_reg8
...
# rho_val = regressionData['rho_val']
c_reg7, res7, eta7, reg_corner7, loss_history, gamma_history, iteration_history, xreg7_history = Tikh_auto_basis_SGD_nesterov_normalized(A, f, A_ts, f_ts, G_D1, scale_value, rho_val, method_our)
x_reg7 = G_D1 @ c_reg7
##compute the error
#erro results
error_history = [ np.sqrt(dr * rho_val.T @ (((G_D1 @ x7).ravel() - K_true_val.T) ** 2)) for x7 in xreg7_history]
...
# regressionData['L2'] = G_D1 @ np.diag(rho_val) @ G_D1.T * dx_coarse  #G_D1 @ np.diag(rho_val) @ G_D1.T * dx
# # ||phi||_{l^2}^2 = c^T * Gbar * Gbar^T * c,
# regressionData['l2'] =  np.eye(*G_D1.shape) #G_D1 @ G_D1.T
...
# # Gbar-RKHS
# method = 'Tikh'  # method can be 'tsvd', 'Tikh', 'dsvd'
# c_reg6L, res6L, eta6L, reg6_cornerL = Tikh_Lcurve(A, f, L2, method)
# x_reg6L = G_D1 @ c_reg6L

# c_reg6l, res6l, eta6l, reg6_cornerl = Tikh_Lcurve(A, f, l2, method)
# x_reg6l = G_D1 @ c_reg6l

# # GCV
# c_reg8L, G8L, reg8_cornerL = Tikh_GCV(A, f, L2, method)
# x_reg8L = G_D1 @ c_reg8L
...
# scale_valueL = 1
# method_our = 'BiSNGD' 
# c_reg7L, res7L, eta7L, reg_corner7L, loss_historyL, gamma_historyL, iteration_historyL, xreg7_historyL = Tikh_auto_basis_SGD_nesterov_norm(A, f, A_ts, f_ts, L2, scale_valueL, rho_val, method_our)
# x_reg7L = G_D1 @ c_reg7L

# error_historyL = [ np.sqrt(dr * rho_val.T @ (((G_D1 @ x7_L).ravel() - K_true_val.T) ** 2)) for x7_L in xreg7_historyL]
# # Plot loss over iterations
# plt.figure()
...
# import os
# datafolder = os.path.join(add_mypathlevy.data_folder, f"FDM{method_our}_sinx_dx_coarse{dx_coarse}_dx{dx}_dt{dt}_N{N}_supp_u{obsInfo['supp_H'][-1]}_t_range{t_range[-1]}_kernel_type{kernel_type}")
...
# np.savez(os.path.join(datafolder, f"bikernel_Gbar_scale{scale_value}.npz"), G_D1=G_D1, rho_val = rho_val, xreg7_history = xreg7_history, A=A)
# np.savez(os.path.join(datafolder, f"bikernel_uxfx_scale{scale_value}.npz"), p_val=p_val, f_val=f_val, x_mesh = obsInfo['x_mesh_data'],fx_vec = regressionData['fx_vec'])
