...
def Tikh_auto_basis_SGD_nesterov_normalized(A, b, A_ts, b_ts, B, scale_value, rho_val, method):
    ...
    for it in range(1, num_iterations + 1):
        ...
        # max_grad_norm = 1e3
        # grad_norm = np.linalg.norm(dF_dgamma)
        # if grad_norm > max_grad_norm:
        #     dF_dgamma = dF_dgamma * (max_grad_norm / (grad_norm + eps))
        ...
        # Gradient norms and directions
        gradient_norm_history.append(np.linalg.norm(dF_dgamma))
        gamma_update_direction.append(dF_dgamma_normalized.copy())
        # Check stability
        if it > 1:
    ...
    return x_reg, res, eta, lambd, loss_history, gamma_history, iteration_history, xreg7_history, gradient_norm_history, gamma_update_direction, D
